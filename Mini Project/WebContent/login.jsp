...
<body>
	...
	<form action=validateLogin.jsp?type=doctor onsubmit="return validateDocLogin()" method=POST>
		...
		<div id=dname_req style="color: red; display: none;">*User name is required</div>
		<div>Password:</div>
		<input type=password name=dpass id=dpass > 
		<div id=dpass_req style="color: red; display: none;">*Password is required</div>
		<br><input type=submit value=Login >
		<a href=signup.jsp >Register Here</a>
	</form>
	
	<!-- Patient Login Form -->
	<form action=validateLogin.jsp?type=patient onsubmit="return validatePatientLogin()" method=POST>
		<h3>Patient Login</h3>
		<div>User Name:</div>
		<input type=text name=pname id=pname >
		<div id=pname_req style="color: red; display: none;">*Patient name is required</div>
		<div>Mobile:</div>
		<input type=number name=pmobile id=pmobile > 
		<div id=pmobile_req style="color: red; display: none;">*Mobile number is required</div>
